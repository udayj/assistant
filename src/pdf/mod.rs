...
use crate::quotation::{QuotedItem, QuotationResponse};
...
pub fn create_quotation_pdf(
    ...
    let (doc, page1, layer1) = PdfDocument::new("Quotation", Mm(PAGE_WIDTH_MM), Mm(PAGE_HEIGHT_MM), "Layer 1");
    ...
    add_header_to_page(&doc, &current_layer, quotation_number, date, &font)?;
    ...
    // Add table headers
    add_table_headers(&current_layer, &font_bold, current_y, col_item, col_qty, col_rate, col_amount, table_width);
    ...
    for item in &quotation.items {
        ...
        if current_y - row_height < 50.0 { // Leave space for totals
        ...
        // Add item data
        add_item_row(&current_layer, &font, &lines, item, current_y, row_height, 
                    col_item, col_qty, col_rate, col_amount);
    ...
    // Add totals section
    current_y -= 20.0;
    add_totals_section(&current_layer, &font, &font_bold, quotation, current_y, col_amount + 30.0);
...
fn add_table_headers(
    ...
    layer.use_text("Qty (Mtr)", 10.0, Mm(col_qty + 2.0), Mm(y_pos - 4.0), font_bold);
    layer.use_text("Rate/mtr.", 10.0, Mm(col_rate + 2.0), Mm(y_pos - 4.0), font_bold);
    layer.use_text("Amount Rs.", 10.0, Mm(col_amount + 2.0), Mm(y_pos - 4.0), font_bold);
    ...
    draw_vertical_line(layer, col_item + table_width, y_pos + 5.0, ROW_HEIGHT_MM+10.0);
}
...
fn draw_table_border(layer: &PdfLayerReference, x: f64, y: f64, width: f64, height: f64) {
    draw_horizontal_line(layer, x, y, width);
